...
from google.generativeai.types import (
    RequestOptions as GoogleRequestOptions,
    HarmCategory,
    HarmBlockThreshold,
)
...
MODEL_INFO = {
    ...
    "ollama": {
        ...
        "llama3.1:8b": {"input_price": 0.0, "output_price": 0.0, "size": "8B"},
        "llama3.2:latest": {"input_price": 0.0, "output_price": 0.0, "size": "3B"},
        "phi3:latest": {"input_price": 0.0, "output_price": 0.0, "size": "3.8B"},
        "qwen2.5-coder:14b": {"input_price": 0.0, "output_price": 0.0, "size": "14B"},
    },
    "novita": {
        "deepseek-r1:671b": {
            "input_price": 0.70,
    ...
    "deepseek": {
        "deepseek-v3": {"input_price": 0.27, "output_price": 1.10, "api_name": "deepseek-chat"},
        "deepseek-r1:671b": {"input_price": 0.55, "output_price": 2.19, "api_name": "deepseek-reasoner"},
    },
}


def default_ollama_num_ctx(model: str) -> int:
    """
    Pick a reasonable default context length per local model.
    Note: actual max depends on the GGUF/model config and your GPU/RAM.
    """
    m = (model or "").lower()
    if "qwen2.5-coder:14b" in m:
        return 12288
    if "llama3.1:8b" in m:
        return 8192
    if "phi3" in m:
    ...
    if provider is None:
        ...
        if is_open_source:
            ...
            for p, models in MODEL_INFO.items():
                if model in models:
                    return p
            supported_models = "\n".join(f"- {p}: {', '.join(models.keys())}" for p, models in MODEL_INFO.items())
            raise ValueError(
                f"Could not detect a provider for model: {model}.\nSupported models:\n{supported_models}"
            )
...
class ModelConfig:
    ...
    def create(
        ...
        provider_configs = {
            ...
            "ollama": {
                ...
                "extra_message_params": {
                    "num_ctx": default_ollama_num_ctx(model),
                },
            },
            "novita": {
                "max_retries": 3,
        ...
        if provider == "openai":
            if not config.get("api_key") or not config.get("organization"):
                raise ValueError(
        ...
        elif provider != "ollama" and not config.get("api_key"):
        ...
        return cls(model_name=model, provider=provider, **config)
...
def get_llm_client(args: argparse.Namespace, logger: Optional["Logger"] = None) -> "LLMClient":
    ...
    if provider == "anthropic" and getattr(args, "thinking", False):
        ...
        config.extra_message_params.setdefault(
            "thinking",
...
class LLMClient(ABC):
    ...
    def _format_response(
        self, content: str, input_tokens: int, output_tokens: int, raw_response: Any
    ) -> Dict[str, Any]:
    ...
    def handle_exception(self, e: Exception) -> Dict[str, Any]:
        ...
        return {"content": "", "usage": {}, "raw_response": None}
        ...
        try:
            ...
            request_options = GoogleRequestOptions(
                retry=g_retry.AsyncRetry(
                    initial=8.0,
                    multiplier=2.0,
                    maximum=self.timeout_per_retry,
...
class OllamaClient(LLMClient):
    def _initialize_client(self):
        ...
        self.chat_params = {
            "num_ctx": extra_message_params.pop("num_ctx", 8192),
        }

        host = os.getenv("OLLAMA_HOST", "").strip()  # e.g. http://127.0.0.1:11434
        extra_client = self.config.extra_client_params or {}
        if host:
            extra_client = {**extra_client, "host": host}

        self.client = AsyncOllama(**extra_client)

    @with_backoff
    async def generate_completion(self, messages: List[Dict[str, str]]) -> Dict[str, Any]:
        """
        System prompt can be included as a message with role 'system' in Ollama chat.
        """
        try:
            ...
            if not content.strip():
                # trigger backoff retry (decorator catches Exception)
